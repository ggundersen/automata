...
public class epsnfa{
 ...
 int incCapacity()
 {
  ...
  /* For definitions, you can refer to slides 14-16 of lecture 5 (Regular Expressions).
   * You are expected to implement the functons that appear in diagram form in the slides.
   * In each case, you should assign new states for the start state and final state of the resulting ?-NFA.
   * You can do this via method:
   ...
   *   or addEdge(a,epssymbol,b); //add an edge with symbol ? from a to b
   * Basically, the whole program, including what you write, will read regular expressions and transform them one by one into an ?-NFA.
   * Then the program will check whether the ?-NFA you construct is correct.
   */
  ...
  int[] union(int s1, int t1, int s2, int t2) {
    ...
    int nt = incCapacity();
    addEdge(t1, epssymbol, nt);
    addEdge(t2, epssymbol, nt);
